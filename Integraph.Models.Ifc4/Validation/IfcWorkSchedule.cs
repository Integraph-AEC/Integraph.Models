	...
	public partial class IfcWorkSchedule : IExpressValidatable
	{
		...
		public bool ValidateClause(IfcWorkScheduleClause clause) {
			...
			try
			{
				...
				var log = Validation.ValidationLogging.CreateLogger<Integraph.Models.Ifc4.ProcessExtension.IfcWorkSchedule>();
				log?.LogError(string.Format("Exception thrown evaluating where-clause 'IfcWorkSchedule.{0}' for #{1}.", clause,EntityLabel), ex);
			}
			return retVal;
		}
