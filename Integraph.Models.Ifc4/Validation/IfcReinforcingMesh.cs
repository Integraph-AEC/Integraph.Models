	...
	public partial class IfcReinforcingMesh : IExpressValidatable
	{
		...
		public bool ValidateClause(IfcReinforcingMeshClause clause) {
			...
			try
			{
				...
				var log = Validation.ValidationLogging.CreateLogger<Integraph.Models.Ifc4.StructuralElementsDomain.IfcReinforcingMesh>();
				log?.LogError(string.Format("Exception thrown evaluating where-clause 'IfcReinforcingMesh.{0}' for #{1}.", clause,EntityLabel), ex);
			}
			return retVal;
		}
