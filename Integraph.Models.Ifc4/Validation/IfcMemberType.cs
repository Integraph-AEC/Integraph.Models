			...
			try
			{
				switch (clause)
				{
					case IfcMemberTypeClause.CorrectPredefinedType:
						retVal = (PredefinedType != IfcMemberTypeEnum.USERDEFINED) || ((PredefinedType == IfcMemberTypeEnum.USERDEFINED) && Functions.EXISTS(this/* as IfcElementType*/.ElementType));
						break;
				}
			} catch (Exception  ex) {
				var log = Validation.ValidationLogging.CreateLogger<Integraph.Models.Ifc4.SharedBldgElements.IfcMemberType>();
				log?.LogError(string.Format("Exception thrown evaluating where-clause 'IfcMemberType.{0}' for #{1}.", clause,EntityLabel), ex);
			}
			return retVal;
		}
