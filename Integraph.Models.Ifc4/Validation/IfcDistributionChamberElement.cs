	...
	public partial class IfcDistributionChamberElement : IExpressValidatable
	{
		...
		public bool ValidateClause(IfcDistributionChamberElementClause clause) {
			...
			try
			{
				...
				var log = Validation.ValidationLogging.CreateLogger<Integraph.Models.Ifc4.SharedBldgServiceElements.IfcDistributionChamberElement>();
				log?.LogError(string.Format("Exception thrown evaluating where-clause 'IfcDistributionChamberElement.{0}' for #{1}.", clause,EntityLabel), ex);
			}
			return retVal;
		}
