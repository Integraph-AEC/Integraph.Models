...
namespace Integraph.Models.Ifc4.HvacDomain
{
	...
	{
		public enum IfcEvaporatorClause
		{
		...
		public bool ValidateClause(IfcEvaporatorClause clause) {
			var retVal = false;
			try
			{
				switch (clause)
				...
				var log = Validation.ValidationLogging.CreateLogger<Integraph.Models.Ifc4.HvacDomain.IfcEvaporator>();
				log?.LogError(string.Format("Exception thrown evaluating where-clause 'IfcEvaporator.{0}' for #{1}.", clause,EntityLabel), ex);
			}
			return retVal;
		}
		...
		public override IEnumerable<ValidationResult> Validate()
		{
			...
			if (!ValidateClause(IfcEvaporatorClause.CorrectPredefinedType))
			...
			if (!ValidateClause(IfcEvaporatorClause.CorrectTypeAssigned))
