	...
	public partial class IfcConstraintRelationship : IExpressValidatable
	{
		...
		public bool ValidateClause(IfcConstraintRelationshipClause clause) {
			...
			try
			{
				switch (clause)
				{
					case IfcConstraintRelationshipClause.WR11:
						retVal = Functions.SIZEOF(RelatedConstraints.Where(temp => Object.ReferenceEquals(temp, RelatingConstraint))) == 0;
						break;
				}
			} catch (Exception ex) {
				var log = Validation.ValidationLogging.CreateLogger<Integraph.Models.Ifc2x3.ConstraintResource.IfcConstraintRelationship>();
				log?.LogError(string.Format("Exception thrown evaluating where-clause 'IfcConstraintRelationship.{0}' for #{1}.", clause,EntityLabel), ex);
			}
			return retVal;
		}
